...
- Server tests: `cd geocoding-server && node --test` — **not** `npm test`,
  that script is a placeholder stub. 250 tests, same throwaway-database-
...
- `master` is a protected branch on GitHub: no direct pushes (enforced
  for everyone, including repo admins), only merges from a pull request
  that has both an approving review and a passing CI run. CI is defined
  in `.github/workflows/ci.yml`; it runs `pytest`, `geocoding-server`'s
  `node --test`, and each of `ui/shared`/`ui/desktop`'s `npm test` on
  every PR (no job for `ui/mobile`, which has no test files yet — see
  the Desktop app tests bullet above). The
  `.github/actions/setup-postgres` composite action gives the runner a
  passwordless `my_ai` Postgres superuser role so the existing
  hardcoded-`my_ai` DSNs in `test/helpers.js`/`tests/conftest.py` work
  unmodified in CI.
- Work happens on a branch named `feature/<short-slug>` (e.g.
  `feature/road-alerts-digest`), never directly on `master`. Open a PR
  as soon as the branch is pushed — a draft PR is fine for
  work-in-progress, since the point is visibility, not a finished diff.
- The repo owner reviews and merges every PR themselves; an agent
  working in this repo opens the PR and describes the change, but does
  not approve or merge its own work.
- Keep PRs the same size as a good commit — small and single-purpose
  (see "Workflow / repo etiquette" above). A PR that bundles unrelated
  changes is harder to review and more likely to get bounced back.
